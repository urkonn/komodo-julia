# Registers the Julia language in Komodo.

import logging
from koUDLLanguageBase import KoUDLLanguage

log = logging.getLogger("koJuliaLanguage")
#log.setLevel(logging.DEBUG)

def registerLanguage(registry):
    log.debug("Registering language Julia")
    registry.registerLanguage(KoJuliaLanguage())

class KoJuliaLanguage(KoUDLLanguage):

    # ------------ Komodo Registration Information ------------ #

    name = "Julia"
    lexresLangName = "Julia"
    _reg_desc_ = "%s Language" % name
    _reg_contractid_ = "@activestate.com/koLanguage?language=%s;1" % name
    _reg_categories_ = [("komodo-language", name)]
    _reg_clsid_ = "c7a2ab37-bd5d-4f51-8001-74cc79e4be9c"
    ...
    #           'M': 'XML',             #   XML
    #        }
    lang_from_udl_family = {'SSL': 'Julia'}
