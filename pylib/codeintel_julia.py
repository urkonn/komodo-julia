...
lang = "Julia"
log = logging.getLogger("codeintel.julia")
#log.setLevel(logging.DEBUG)


# These keywords are copied from "Julia-mainlex.udl" - be sure to keep both
# of them in sync.
keywords = [
    # Keywords
        "function",
...
# lexers. You must define a Lexer class for lang Julia. If Komodo's
# scintilla lexer for Julia is UDL-based, then this is simply:
#
#   from codeintel2.udl import UDLLexer
#   class JuliaLexer(UDLLexer):
#       lang = lang
#
# Otherwise (the lexer for Julia is one of Komodo's existing C++ lexers
...
#   class JuliaLexer(Lexer):
#       lang = lang
#       def __init__(self):
#           self._properties = SilverCity.PropertySet()
#           self._lexer = SilverCity.find_lexer_module_by_id(ScintillaConstants.SCLEX_JULIA)
#           self._keyword_lists = [
...
# examples of usage.
class JuliaLangIntel(LangIntel):
    lang = lang
...
# Julia content and returns a description of the code in that file.
# See "cile_julia.py" for more details.
#
# The CILE Driver is a class that calls this CILE. If Julia is
...
class JuliaCILEDriver(UDLCILEDriver):
    lang = lang

    def scan_purelang(self, buf):
        import cile_julia
        return cile_julia.scan_buf(buf)
...
def register(mgr):
    ...
    mgr.set_lang_info(
        lang,
        silvercity_lexer=JuliaLexer(),
        buf_class=JuliaBuffer,
        langintel_class=JuliaLangIntel,
        import_handler_class=None,
        cile_driver_class=JuliaCILEDriver,
