...
"""A Code Intelligence Language Engine for the Julia language.
...
Module Usage:
    from cile_julia import scan
    mtime = os.stat("bar.julia")[stat.ST_MTIME]
    content = open("bar.julia", "r").read()
    scan(content, "bar.julia", mtime=mtime)
"""
...
class JuliaCILEError(CILEError):
    pass
...
log = logging.getLogger("cile.julia")
...
def scan_buf(buf, mtime=None, lang="Julia"):
    """Scan the given JuliaBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {JuliaBuffer} is the Julia buffer to scan
    ...
    """
    # Dev Notes:
    # - This stub implementation of the Julia CILE return an "empty"
    #   summary for the given content, i.e. CIX content that says "there
    #   are no code elements in this Julia content".
    # - Use the following command (in the extension source dir) to
    #   debug/test your scanner:
    #       codeintel scan -p -l Julia <example-Julia-file>
    ...
    # Dev Note:
    # This is where you process the Julia content and add CIX elements
    ...
    #   Use the "codeintel html -b <example-Julia-file>" command as
    #   a debugging tool.
    # - "buf.accessor.text" is the whole content of the file. If you have
    #   a separate tokenizer/scanner tool for Julia content, you may
    #   want to use it.
